...
public class data {
    ...
    public static void addTpa(UUID p, UUID tg) {
        ...
        BukkitTask newRq = new BukkitRunnable() {
            ...
            public void run() {
                ...
                if (p1 != null) utils.send(p1, "&c你對" + Bukkit.getOfflinePlayer(tg).getName() + "發出的請求已過期!");
            }
    ...
    public static void addTpa(Player p, Player tg) {
        addTpa(p.getUniqueId(), tg.getUniqueId());
    }
    public static void remTpa(UUID p, UUID tg, Boolean ended) {
        List<HashMap<UUID, BukkitTask>> cL = getTpa(p);
    ...
    public static void remTpa(Player p, Player tg, Boolean ended) {
        remTpa(p.getUniqueId(), tg.getUniqueId(), ended);
    }
}
